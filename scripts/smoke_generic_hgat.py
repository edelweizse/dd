#!/usr/bin/env python
"""
Run an end-to-end smoke test for GenericHGAT with strict split reuse.
...
7. Compare GenericHGAT against selected baselines on the same split

Usage:
    PYTHONPATH=. python scripts/smoke_generic_hgat.py
"""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="End-to-end smoke test for GenericHGAT")
    ...
    parser.add_argument(
        ...
        help=f"Comma-separated baselines to compare against GenericHGAT. Available: {', '.join(BASELINE_NAMES)}",
    )
    ...
    ts = int(time.time())
    work_dir = Path(args.work_dir) if args.work_dir else Path(f"/tmp/dd_generic_hgat_e2e_{ts}")
    ...
    print("GenericHGAT smoke configuration")
    ...
    print("Building GenericHGAT model...")
    ...
    expected_models = {"main_generic_hgat", *requested_baselines}
    ...
    print("GenericHGAT smoke test passed")
