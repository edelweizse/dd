...
- HGATPredictor: Full model with encoder and bilinear decoder
...
class HGATPredictor(nn.Module):
    """
    Heterogeneous Graph Attention (HeteroGAT) encoder + CD link decoder.
...
def infer_hgat_hparams_from_state(state: Dict[str, torch.Tensor]) -> Dict[str, Union[int, Dict[str, int]]]:
    """Infer HGATPredictor init kwargs from a checkpoint ``model_state``."""
    attn_keys = [k for k in state if k.startswith('convs.') and '.attn_' in k]
    if not attn_keys:
        raise ValueError('Checkpoint state has no HGAT attention tensors (convs.*.attn_*).')
...
def create_model_from_data(
    ...
    """
    Create an HGATPredictor model from HeteroData.
    ...
    Returns:
        Initialized HGATPredictor model.
    """
